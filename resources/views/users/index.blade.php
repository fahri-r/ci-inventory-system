        ...
        <div class="page-title">
            <div class="row">
                <div class="col-12 col-md-6 order-md-1 order-last">
                    <h3>
                        {{ $title }}
                        <a href="{{ route('users.create') }}" class="btn btn-primary rounded-pill"><i
                ...
                <div class="card-body">
                    <table class="table table-striped" id="table1">
                        ...
                        <tbody>
                            @foreach ($users as $u)
                            <tr>
                                <td>{{ $u->userMeta->firstname, $u->userMeta->lastname}}</td>
                                <td>{{ $u->email }}</td>
                                <td>{{ $u->userMeta->gender == 1 ? 'Male' : 'Female'}}</td>
                                <td>{{ $u->group->name }}</td>
                                <td>
                                    <a href="{{ route('users.show', ['user'=>$u->id]) }}" class="btn btn-primary"><i
                                            class="fas fa-eye"></i></a>
                                    <a href="{{ route('users.edit', ['user'=>$u->id]) }}" class="btn btn-warning"><i
